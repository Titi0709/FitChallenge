    ...
    <v-container>
      ...
      <div v-if="paginatedPart.length">
        <div v-for="p in paginatedPart" :key="p.id_participation" class="progression-row">
          <v-img :src="`/storage/${p.defi.image}`" class="progression-img" />
          <div class="progression-content">
            ...
            <div class="progression-btns">
              ...
              <template v-else>
                <v-btn color="success"
                  @click="openDialog(p.id_participation, 'valider', 'participation')">VALIDÉ</v-btn>
                <v-btn color="error"
                  @click="openDialog(p.id_participation, 'abandonner', 'participation')">ABANDONNER</v-btn>
                <v-btn color="#c62e43" class="voir-plus-btn" @click="$inertia.visit(`/defi/${p.defi.id_defi}`)">Voir
                  plus</v-btn>
              </template>
            </div>
          </div>
      ...
      <div v-if="paginatedDefis.length">
        <div v-for="d in paginatedDefis" :key="d.id_defi" class="progression-row">
          ...
          <div class="progression-content">
            ...
            <div class="progression-btns">
              <v-btn color="#c62e43" class="voir-plus-btn" @click="$inertia.visit(`/defi/${d.id_defi}`)">Voir
                plus</v-btn>
